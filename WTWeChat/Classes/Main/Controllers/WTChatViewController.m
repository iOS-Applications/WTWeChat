...
#import "HttpTool.h"
@interface WTChatViewController ()<UITableViewDataSource,UITableViewDelegate,NSFetchedResultsControllerDelegate,UITextViewDelegate,UINavigationControllerDelegate, UIImagePickerControllerDelegate>{
...
@implementation WTChatViewController
...
-(void)keyboardWillShow:(NSNotification *)noti{
    ...
    //竖屏{{0, 0}, {768, 264}
    //横屏{{0, 0}, {352, 1024}}
    // 如果是ios7以下的，当屏幕是横屏，键盘的高度是size.with
...
#warning  代码实现自动布局,要设置下面的属性为NO
    ...
    //创建输入框view
    WTInputView *inputView=[WTInputView inputView];
...
-(void)loadMsgs{
    ...
    // 上下文
    NSManagedObjectContext *context = [WTXMPPTool sharedWTXMPPTool].msgStorage.mainThreadManagedObjectContext;
    // 请求对象
    ...
    // 1.当前登录用户的JID的消息
    // 2.好友的Jid的消息
    NSPredicate *pre = [NSPredicate predicateWithFormat:@"streamBareJidStr = %@ AND bareJidStr = %@",[WTUserInfo sharedWTUserInfo].jid,self.friendJid.bare];
    WTLog(@"%@",pre);
    ...
    WTLog(@"%@",_resultsContr.fetchedObjects);
    if (err) {
        WTLog(@"%@",err);
    }
}
...
-(void)textViewDidChange:(UITextView *)textView{
    ...
    WTLog(@"textView的content的高度 %f",contentH);
...
-(void)sendMsgWithText:(NSString *)text bodyType:(NSString *)bodyType{
    ...
    [msg addBody:text];
    WTLog(@"%@",msg);
    [[WTXMPPTool sharedWTXMPPTool].xmppStream sendElement:msg];
}
...
-(void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info{
    ...
    // 1.取文件名 用户名 + 时间(201505121520)年月日时分秒
